...
function setup() {
  ...
  // deal with mobile device nuances
  md = { mobile: function () { return /Mobi|Android|iPhone|iPad/i.test(window.navigator.userAgent) ? 'mobile' : null; } };
  /*
  if (md.mobile()) {
      mobileMode = true;
...
function draw() {
  ...
  if (result !== 0 && trainingVersion) { // someone has lost
    ...
    if (trainingVersion) {
      if (window.jQuery) $("#nn_gene").text(JSON.stringify(trainer.getChromosome()));
  ...
  if (sunY < height+sunR) {
    ...
    for (var i=0; i<5; i++) {
      ...
      ellipse(sunX, sunY, sunR-i*sunR/30, sunR-i*sunR/30);
    }
    pop();
  }
