...
- **This is a git repo** (branch `main`, remote `github.com/mars-college/mars.college` — the repo lives in the Mars College org now; the old `genekogan/` URL still redirects) **and it is public** — the only versioned subtree in the Mars archive, so it's the one place a bad commit becomes publicly visible. Check diffs for secrets before committing.
