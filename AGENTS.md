...
- **This is a git repo** (branch `main`, remote `github.com/genekogan/mars.college`) **and it is public** — the only versioned subtree in the Mars archive, so it's the one place a bad commit becomes publicly visible. Check diffs for secrets before committing.
